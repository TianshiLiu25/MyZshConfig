...
# machine specific
local MACHINE_SPECIFIC_FILE_BEFORE="${HOME}/LocalConfig/local_env_before.sh"
local MACHINE_SPECIFIC_FILE_AFTER="${HOME}/LocalConfig/local_env_after.sh"
alias rf="source $MACHINE_SPECIFIC_FILE_AFTER"
alias ef="vi $MACHINE_SPECIFIC_FILE_AFTER"

source $MACHINE_SPECIFIC_FILE_BEFORE
...
# proxy
function proxy_on {
    proxy_on_ip=$(hostname -I | sed 's/ *$//')
    export http_proxy=http://$proxy_on_ip:7890;
    export https_proxy=http://$proxy_on_ip:7890;
}
function proxy_off {
    unset http_proxy;
    unset https_proxy;
...
source $MACHINE_SPECIFIC_FILE_AFTER